...
class MyService {
    ...
    List<Product> transformToStandardAndDtoTo30() {
        def toStandard = { it.copyWith(type: 'standard') }
        def dtoTo30 = { it.copyWith(dto: 0.3) }

        return products.collect(dtoTo30 << toStandard)
    ...
    List<Product> transformToLuxuryAndDtoToZero() {
        def toLuxury = { it.copyWith(type: 'luxury') }
        def dtoTo0 = { it.copyWith(dto: 0) }

        return products.collect(dtoTo0 << toLuxury)
        // No se puede recorrer la lista mas de una vez
        // las expresiones deben reutilizarse
        // No se pueden utilizar bucles anidados
    }
...
myService.transformToStandardAndDtoTo30()
myService.transformToLuxuryAndDtoToZero()