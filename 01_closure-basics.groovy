...
Closure<Integer> suma = { Integer a, Integer b ->
    return a + b
}
...
Closure<Boolean> isEven = { Integer number ->
    return (number % 2 == 0)
}

Closure<Boolean> insideOneToOneHundred = { Integer number ->
    return (number >= 1 && number <= 100)
}

Closure<Boolean> isEvenAndInsideOneToOneHundred = { Integer number ->
    return isEven(number) && insideOneToOneHundred(number)
}

assert (1..100).findAll(isEvenAndInsideOneToOneHundred)


// [3] CREAR UN METODO QUE PUEDA SERVIR PARA FILTRAR LOS NUMEROS IMPARES DEL
// 1 AL 100
Closure<Boolean> isOdd = { Integer number ->
    !isEven(number)
}

Closure<Boolean> isOddAndInsideOneToOneHundred = { Integer number ->
    return isOdd(number) && insideOneToOneHundred(number)
}

assert (1..100).findAll(isOddAndInsideOneToOneHundred)


// [4] DECLARAR UNA CLOSURE PARA MULTIPLICAR (COLLECT) TODOS LOS NUMEROS DEL
// 1 AL 100 CONTENIDOS EN UNA LISTA, POR EL NUMERO 2
Closure<Integer> twoTimes = { Integer number -> number * 2 }

println ((1..100).collect(twoTimes))
...
Closure<Integer> threeTimes = { Integer number -> number * 3 }

println ((1..100).collect(threeTimes))